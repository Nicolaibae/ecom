...
import { MessageResType } from "src/shared/models/reponse.model";
...
export class PaymentRepo {
    ...
    private totalPriceOrder(orders: OrderIncludeProductSKUSnapshotType[]): number {
        return orders.reduce((total, order) => {
            const orderTotal = order.items.reduce((totalPrice, skuOrder) => {
                return totalPrice + skuOrder.skuPrice * skuOrder.quantity
            }, 0)
