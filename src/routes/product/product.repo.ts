...
export class ProductRepo {
    ...
    async update({
        ...
        const getDataSku = await this.prismaService.sKU.findMany({
            where: {
                productId: id,
                deletedAt: null
            }
        ...
        const skusToDelete = getDataSku.filter((sku) => dataSkus.every((dataSku) => dataSku.value !== sku.value))
        const skuIdsToDelete = skusToDelete.map(sku => sku.id)
        // 3. Mapping ID vào trong data payload (dataSkus)
        const skusWithId = dataSkus.map((dataSku) => {
            const matchSku = getDataSku.find((sku) => sku.value === dataSku.value)
            return {
                ...dataSku,
                id: matchSku ? matchSku.id : null
            }
        })
        ...
        // 5.Tìm các skus để thêm mới ( SKU không tồn tại trong DB nhưng có trong data payload thì sẽ được thêm mới)
